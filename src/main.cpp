...
#if defined(NDEBUG)
# error "Konsotami cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xeaaded9cd3b06637ee9955e8897e8b3fd204bfa38eaada1974be9a5826ed54f9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Konsotami: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Konsotami Signed Message:\n";
...
{
    // Konsotami: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Konsotami: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Konsotami: 1 day
static const int64 nTargetSpacing = 5 * 60; // Konsotami: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Konsotami: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Konsotami: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf8bfd5c3d50ea77ec5a244d86df1383e14c82ff8c84b2c1f2832828a6d4b13bc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "quatre jour avant le trente troisieme anniversaire de la mort de mon pere";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1540823011;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087502792;

        if (fTestNet)
        {
            block.nTime    = 1540822984;
            block.nNonce   = 386254378;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1f7f3c359248ab057e4d8e9cb37f6fb0579a31e48034bd3bdd31683c89295664"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Konsotami: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KonsotamiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KonsotamiMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KonsotamiMiner(CWallet *pwallet)
{
    printf("KonsotamiMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("konsotami-miner");
    ...
    try { loop {
        ...
        printf("Running KonsotamiMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KonsotamiMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KonsotamiMiner, pwallet));
}
